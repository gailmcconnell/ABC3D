# -*- coding: utf-8 -*-
"""
Created on Fri Feb  6 09:24:16 2026
...
# =========================
# USER SWITCH (Between synthetic data, file, i.e. your data, and both. Synthetic data is for verifying code function)
# =========================
RUN_MODE = "file"     # "file" | "synthetic" | "both"

TIF_PATH = Path(
    r"C:\Users\acp98112\OneDrive - University of Strathclyde\Documents\Research\ABC3D\Keio\M9\BW25113 ydgD\BW25113_ydgD_5um_stack5.tif"
)
...
# FFT controls
FFT_HF_CUT = 0.25
FFT_CROP_MARGIN = 8     # voxels padding around ROI bbox for FFT (set 0 to disable)
...
def fft_energy_features_3d(vol: np.ndarray, roi: np.ndarray, hf_cut: float = 0.25, crop_margin: int = 8) -> dict:
    """
    16GB-RAM friendly FFT features:
    - Crop to ROI bounding box (with margin) to shrink FFT size massively
    - Use real FFT (rfftn) to halve spectrum storage
    - No fftshift, no radius arrays, no np.abs() temporaries
    - Hann window applied separably (no 3D window array)
    """
...
def wavelet_energies_3d(vol: np.ndarray, roi: np.ndarray, wavelet: str = "db2") -> Dict[str, float]:
    if np.count_nonzero(roi) == 0:
        return {k: float("nan") for k in [
            ...
            "LLL_energy","LLH_energy","LHL_energy","LHH_energy",
            "HLL_energy","HLH_energy","HHL_energy","HHH_energy"
        ]}
    ...
    energies = {name_map[k]: float(np.sum(np.asarray(arr, np.float64) ** 2)) for k, arr in coeffs.items()}

    energies["LL_energy"] = energies.get("LLL_energy", float("nan"))
    energies["HH_energy"] = energies.get("HHH_energy", float("nan"))
    energies["LH_energy"] = _safe_mean([energies.get("LLH_energy", np.nan),
                                        energies.get("LHL_energy", np.nan),
                                        energies.get("HLL_energy", np.nan)])
    energies["HL_energy"] = _safe_mean([energies.get("LHH_energy", np.nan),
                                        energies.get("HLH_energy", np.nan),
                                        energies.get("HHL_energy", np.nan)])

    for k in ["LLL_energy","LLH_energy","LHL_energy","LHH_energy","HLL_energy","HLH_energy","HHL_energy","HHH_energy",
              "LL_energy","LH_energy","HL_energy","HH_energy"]:
        energies.setdefault(k, float("nan"))
    return energies
...
def run_file_mode() -> pd.DataFrame:
    ...
    vol = load_volume_as_zyx(TIF_PATH)
    roi = make_binary_mask(vol, threshold=THRESHOLD, invert=INVERT, min_object_size=MIN_OBJECT_SIZE)

    feats = extract_features(vol, roi)
    feats["type"] = TIF_PATH.name
...
if __name__ == "__main__":
    ...
    if RUN_MODE in {"file", "both"}:
        ...
        print("\n=== FILE MODE ===")
        print(df_file[["type", "boxcount_D", "boxcount_r2", "foreground_fraction",
                       "fft_highfreq_energy_ratio", "fft_used_shape_z", "fft_used_shape_y", "fft_used_shape_x"
                       ]].to_string(index=False))

    if RUN_MODE in {"synthetic", "both"}:
        df_syn = synthetic_suite_table()
        print("\n=== SYNTHETIC MODE ===")
        print(df_syn[["type", "boxcount_D", "boxcount_r2", "lacunarity_mean",
                      "shannon_entropy", "renyi_entropy_a2", "fft_highfreq_energy_ratio"
                      ]].to_string(index=False))

    if SAVE_CSV:
        if df_file is not None:
            out_path = OUT_DIR / "features_file.csv"
